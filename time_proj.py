...
class HyTE(Model):
	...
	def load_data(self):
		...
		with open(self.p.dataset,'r') as filein: # 遍历train.txt训练集
			for line in filein:
				train_triples.append([int(x.strip()) for x in line.split()[0:3]]) # 并将<head,rel,tail>三元组存在train_triples中
		...
		self.inp_idx['triple'], self.start_idx['triple'], self.end_idx['triple'] = self.create_id_labels(triple_time,'triple') # 优化时间信息
		#pdb.set_trace()
		print('inp_idx:' + str(len(self.inp_idx['triple'])) + ' start_idx:' + str(len(self.start_idx['triple'])) + ' end_idx:' + str(len(self.end_idx['triple'])))
